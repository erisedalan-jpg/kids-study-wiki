...
_由 `00-元/scripts/stats.py` 生成，共 5839 词条 / 7 学科。_
...
- ⏳ 初中 Phase 2 剩余 3 学科：政治 ~120 / 地理 ~200 / 历史 ~250 (~570 词条)
  - 化学/生物/物理 已全段完成（见上方 ✅ 物理 450 / 化学 520 / 生物 457），不在剩余之列
...
- ✅ 数学扩至全学段（2026-05-22）：小学→初中→高中 梳理 + 纵向贯通
  ...
  - ✅ 去重合并（2026-05-23）：96 组近重复（X/X的、图象/图像等命名碎片）中 **76 组非富内容对已合并**（裸名并入规范件 alias、学段取并、全库链接重定向 数学28+真题索引217 文件、删 76 词条）；数学 1423→1347，断链 6618→6475。引擎 `docs/superpowers/working/merge_dups.py`（含自检）
  - ⏳ 余 23 组「重复件内容更全」待人工/按字数择优并入（`docs/superpowers/working/dedup_plan.md` 全清单）
  - ⏳ 高中逐条体检 backlog（945：1785 前沿裸链 + 889 教材引用）；本轮按约定不强推
  - 全量单测 122 过
